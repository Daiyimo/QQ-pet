...
QQ 宠物怀旧服（v1.2.4）的 Windows 桌宠，AI 能力全部走云端多服务商 LLM。宠物会看屏幕、写日记、录课程、发弹幕，并根据你的作息提醒休息。
...
喂养 / 清洗 / 吃药、打工赚元宝、上学提升学识、商城购物、小游戏、钓鱼、后花园、宠物资料与属性面板、贴边隐藏、鼠标穿透。
...
当前 224 个测试，全部纯 Node 运行、不依赖 Electron，通过依赖注入（时钟 / 随机数 / 存储 / 服务商）隔离外部依赖。
